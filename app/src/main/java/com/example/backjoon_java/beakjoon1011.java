package com.example.backjoon_java;

import java.util.Scanner;

public class beakjoon1011 {
    /*
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i<testCase ; i++) {
            ...
            int distance = input2-input1;
            int answer = 0;

            double root = Math.sqrt(distance);
            int group = root%1 < 0.5 ? (int)(root/1) : (int)(root/1)+1;

            if (distance == 1) {
                answer = 1;
            } else if (distance <= Math.pow(group,2)) {
                answer = group * 2 - 1;
            } else {
                answer = group * 2;
            }
